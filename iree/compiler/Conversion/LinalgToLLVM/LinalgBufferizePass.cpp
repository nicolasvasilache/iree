...
static LogicalResult allocateBuffersForResults(
  ...
  for (auto en : llvm::enumerate(op.getOperation()->getResultTypes())) {
    ...
    if (foldedInitTensor) {
      ...
      if (initTensor.use_empty()) continue;
    ...
    for (auto shapeElement : llvm::enumerate(tensorType.getShape())) {
      if (loopRanges.empty()) loopRanges = op.createLoopRanges(b, loc);
...
LogicalResult convertInterfaceLoadTensorOp(
  ...
  // Potentially duplicate placeholders will be taken care of by CSE.
...
LogicalResult convertInterfaceLoadTensorOp(
  ...
  Value subview = b.create<SubViewOp>(
      loadOp->getLoc(), inferredType, buffer, loadOp.offsets(), loadOp.sizes(),
      loadOp.strides(), loadOp.static_offsets(), loadOp.static_sizes(),
      loadOp.static_strides());
...
LogicalResult convertInterfaceStoreTensorOp(
  ...
  auto inferredType = SubViewOp::inferResultType(
      ...
      extractFromI64ArrayAttr(storeOp.static_strides()));
  Value subview = b.create<SubViewOp>(
      storeOp->getLoc(), inferredType, buffer, storeOp.offsets(),
...
void LinalgLLVMBufferizePass::runOnFunction() {
  ...
  funcOp.walk([&](Operation *op) {
    if (isa<BranchOpInterface>(op) || isa<RegionBranchOpInterface>(op)) {
      if (!isa<scf::ForOp>(op)) fail = true;
      return WalkResult::interrupt();
    }
    return WalkResult::advance();
  });
  if (fail) return signalPassFailure();
  ...
  funcOp.walk([&](Operation *op) {
    ...
    if (auto loadOp = dyn_cast<IREE::HAL::InterfaceLoadTensorTileOp>(op))
      convertInterfaceLoadTensorOp(b, loadOp, bvm);
  });
  ...
  funcOp.walk([&](Operation *op) {
    ...
    if (auto storeOp = dyn_cast<IREE::HAL::InterfaceStoreTensorTileOp>(op))
      convertInterfaceStoreTensorOp(b, storeOp, bvm);
  });
}
