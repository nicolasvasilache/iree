...
// RUN:     --iree-preloaded-transforms=%p/transform_dialect_codegen_bufferize_spec.mlir | \
// RUN: FileCheck %s

// RUN: iree-opt %s  --pass-pipeline="builtin.module(hal.executable(hal.executable.variant(iree-llvmgpu-lower-executable-target)))" \
// RUN:     --iree-codegen-llvmgpu-enable-transform-dialect-jit=false \
// RUN:     --iree-preloaded-transforms=%p/transform_dialect_codegen_foreach_to_gpu_spec.mlir | \
